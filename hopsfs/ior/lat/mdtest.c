...
FILE *fout_mkdir_latency = NULL;
FILE *fout_statdir_latency = NULL;
FILE *fout_rmdir_latency = NULL;
FILE *fout_create_latency = NULL;
FILE *fout_stat_latency = NULL;
FILE *fout_open_latency = NULL;
FILE *fout_unlink_latency = NULL;
...
static void prep_testdir(int j, int dir_iter) {
    ...
    o.testdir[0] = '\0';
    //   sprintf(o.testdir, "/home/heifeng.lwh/ior-master-hdfs-iops/lala");
}
...
static void remove_file(const char *path, uint64_t itemNum) {
    ...
    if (!(o.shared_file && rank != 0)) {
        ...
        double ed = MPI_Wtime();
        fprintf(fout_unlink_latency, "%.9f\n", ed - st);
    }
}
...
static void create_file(const char *path, uint64_t itemNum) {
    ...
    if (o.make_node) {
        ...
        double ed = MPI_Wtime();
        fprintf(fout_create_latency, "%.9f\n", ed - st);
        if (ret != 0)
...
void mdtest_stat(const int random, const int dirs, const long dir_iter, const char *path, rank_progress_t *progress) {
    ...
    for (uint64_t i = 0; i < stop_items; ++i) {
        ...
        */
        ...
        double ed = MPI_Wtime();
        if (dirs) {
            fprintf(fout_statdir_latency, "%.9f\n", ed - st);
        } else {
            fprintf(fout_stat_latency, "%.9f\n", ed - st);
        }
    }
}
...
void directory_test(const int iteration, const int ntasks, const char *path, rank_progress_t *progress) {
    ...
    if (rank == 0) {
        printf("   Directory creation: %14.3f sec, %14.3f ops/sec\n", t[1] - t[0], o.items * size / (t[1] - t[0]));
        fflush(stdout);
    }
    ...
    if (rank == 0) {
        printf("   Directory stat    : %14.3f sec, %14.3f ops/sec\n", t[2] - t[1], o.items * size / (t[2] - t[1]));
        fflush(stdout);
    }
    ...
    if (rank == 0) {
        printf("   Directory removal : %14.3f sec, %14.3f ops/sec\n", t[4] - t[3], o.items * size / (t[4] - t[3]));
        fflush(stdout);
    }
...
void file_test(const int iteration, const int ntasks, const char *path, rank_progress_t *progress) {
    ...
    if (rank == 0) {
        printf("  File creation     : %14.3f sec, %14.3f ops/sec\n", t[1] - t[0], o.items * o.size / (t[1] - t[0]));
        fflush(stdout);
    }
    ...
    if (rank == 0) {
        printf("  File stat         : %14.3f sec, %14.3f ops/sec\n", t[2] - t[1], o.items * o.size / (t[2] - t[1]));
        fflush(stdout);
    }
    ...
    if (rank == 0) {
        printf("  File read         : %14.3f sec, %14.3f ops/sec\n", t[3] - t[2], o.items * o.size / (t[3] - t[2]));
        fflush(stdout);
    }
    ...
    if (rank == 0) {
        printf("  File removal      : %14.3f sec, %14.3f ops/sec\n", t[4] - t[3], o.items * o.size / (t[4] - t[3]));
        fflush(stdout);
    }
...
void summarize_results(int iterations, int print_time) {
    ...
    for (i = start; i < stop; i++) {
        ...
        for (k = 0; k < o.size; k++) {
            for (j = 0; j < iterations; j++) {
                var += pow((mean - all[(k * tableSize * iterations) + (j * tableSize) + i]), 2);
            }
        }
...
void show_file_system_size(char *file_system) {
    ...
    used_file_system_percentage = (1 - ((double)free_file_system_size / (double)total_file_system_size)) * 100;
...
mdtest_results_t *mdtest_run(int argc, char **argv, MPI_Comm world_com, FILE *world_out) {
    fout_mkdir_latency = fopen("/home/heifeng.lwh/latency/mkdir", "w");
    fout_statdir_latency = fopen("/home/heifeng.lwh/latency/statdir", "w");
    fout_rmdir_latency = fopen("/home/heifeng.lwh/latency/rmdir", "w");
    fout_create_latency = fopen("/home/heifeng.lwh/latency/creat", "w");
    fout_stat_latency = fopen("/home/heifeng.lwh/latency/stat", "w");
    fout_open_latency = fopen("/home/heifeng.lwh/latency/open", "w");
    fout_unlink_latency = fopen("/home/heifeng.lwh/latency/unlink", "w");
    ...
    if (rank == 0)
        ShowFileSystemSize(o.testdirpath, o.backend, o.backend_options);
    ...
    fclose(fout_mkdir_latency);
    fclose(fout_statdir_latency);
    fclose(fout_rmdir_latency);
    fclose(fout_create_latency);
    fclose(fout_stat_latency);
    fclose(fout_open_latency);
    fclose(fout_unlink_latency);
