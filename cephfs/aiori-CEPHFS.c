...
static IOR_offset_t CEPHFS_GetFileSize(IOR_param_t *param, MPI_Comm testComm,
    ...
    int ret = ceph_statx(cmount, pfix(testFileName), &stx, CEPH_STATX_ALL_STATS, AT_NO_ATTR_SYNC);
    if (ret < 0) {
        CEPHFS_ERR("ceph_stat failed", ret);
    }
    aggFileSizeFromStat = stx.stx_size;
...
static int CEPHFS_Access(const char *testFileName, int mode, IOR_param_t *param) {
    ...
    struct ceph_statx stx;
    return ceph_statx(cmount, pfix(testFileName), &stx, CEPH_STATX_ALL_STATS, AT_NO_ATTR_SYNC);
}
